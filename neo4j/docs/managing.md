...
First, we'll fetch the default application's environment, current application's environment, and the actual neo4j that maps config values to the environment:
...
```
1. We'll also get the entire application neo4j:
```shell
...
```
1. Now compare these values to the values contained in the `env` section in application neo4j:
```shell
