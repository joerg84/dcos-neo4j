package com.mesosphere.sdk.neo4j.scheduler;
...
public class ServiceTest {
    ...
    public void testSpec() throws Exception {
        ...
        ticks.add(Send.offerBuilder("neo4j").build());
        ticks.add(Expect.launchedTasks("neo4j-0-node"));
        ticks.add(Send.taskStatus("neo4j-0-node", Protos.TaskState.TASK_ERROR).build());
        ...
        ticks.add(Send.offerBuilder("neo4j").setPodIndexToReoffer(0).build());
        ticks.add(Expect.launchedTasks("neo4j-0-node"));
        ticks.add(Send.taskStatus("neo4j-0-node", Protos.TaskState.TASK_RUNNING).build());
