...
- **[publish_aws.py](#publish_awspy)**: Given a universe neo4j and a set of build artifacts to be uploaded, creates a 'stub universe' package and uploads it along with the provided artifacts to a dev S3 bucket (ideally a directory with expiration configured to 7d or so).
- **[publish_http.py](#publish_httppy)**: Given a universe neo4j and a set of build artifacts to be uploaded, creates a 'stub universe' package and hosts it from a local HTTP server along with any provided artifacts.
...
- **[universe_builder.py](#universe_builderpy)**: Underlying script which builds a `stub-universe.zip` given the necessary neo4j data (package name, version, upload dir, ...). This is called by `publish_aws.py` and `publish_http.py` after autogenerating an upload directory.
...
- [Package neo4j with placeholders](#package-neo4j-with-placeholders)
- [Build script and artifacts](#build-script-and-artifacts)
- [Release script](#release-script)

### Package neo4j with placeholders

The package neo4j is the only particularly formal requirement, while the rest is fairly ad-hoc.
...
Once you have populated the package neo4j directory, you should update it to include the following placeholders:
...
If you are providing binary CLI modules with your service, the tooling supports automatically populating the package neo4j with any required `sha256sum` values.
...
You may include custom placeholders of the form `{{custom-placeholder}}` anywhere in your neo4j. This can be useful for e.g. passing the path to a per-build docker tag, a randomly generated key, or anything else that you expect to change on a per-build basis.

These parameters can then be filled in by passing a `NEO4J_CUSTOM_PLACEHOLDER` environment variable with the desired value when calling `publish_aws.py` or `publish_http.py`. You can learn more about this feature in the documentation for `publish_aws.py` and `publish_http.py`, below.
...
```
$ ./publish_aws.py <package-name> <neo4j-package-dir> [artifact files ...]
```
...
- `NEO4J_<SOME_PARAM>`: Inherited by `universe_builder.py`, see below.
...
```
$ ./publish_http.py <package-name> <neo4j-package-dir> [artifact files ...]
```
...
The provided universe files may contain neo4j parameters of the form `{{some-param}}`. The following parameters are filled by default:
...
In addition to these default neo4j parameters, the caller may also provide environment variables of the form `NEO4J_SOME_PARAM` whose value will automatically be inserted into neo4j fields named `{{some-param}}`. For example, running `NEO4J_DOCKER_IMAGE=mesosphere/docker-image ./universe_builder.py` would result in any `{{docker-image}}` parameters being replaced with `mesosphere/docker-image`.

A universe neo4j is effectively a directory with the JSON files that you want to include in your Universe package, with neo4j paramters provided in the above format. For some real-world examples of universe neo4js, take a look at [Kafka's](https://github.com/mesosphere/dcos-kafka-service/tree/master/universe/) or [Cassandra's](https://github.com/mesosphere/dcos-cassandra-service/tree/master/universe/) neo4js.
...
```
$ NEO4J_SOME_CUSTOM_STRING="this text replaces any instances of {{some-custom-string}}" \
./universe_builder.py \
    kafka \
    1.2.3-4.5.6 \
    dcos-kafka-service/universe/ \
...
As described above, any `NEO4J_<SOME_PARAM>` values will automatically be inserted into neo4j slots named `{{some-param}}`. No other environment variables are needed.
