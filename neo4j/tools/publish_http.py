...
def print_help(argv):
    logger.info('Syntax: {} <package-name> <neo4j-package-dir> [artifact files ...]'.format(argv[0]))
    ...
    logger.info('In addition, environment variables named \'NEO4J_SOME_PARAMETER\' will be inserted against the provided package neo4j (with params of the form \'{{some-parameter}}\')')
...
def main(argv):
    ...
    # the package name:
    package_name = argv[1]
    # local path where the package neo4j is located:
