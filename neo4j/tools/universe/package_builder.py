...
class UniversePackageBuilder(object):
    ...
    def _get_neo4j_mapping_for_content(self, orig_content):
        '''Returns a neo4j mapping (dict) for the following cases:
        ...
        - Custom environment params like 'NEO4J_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default neo4j values (may be overridden via eg NEO4J_PACKAGE_VERSION envvars):
        now = time.time()
        neo4j_mapping = {
        ...
        # look for any 'sha256:filename' neo4j params, and get shas for those.
        # this avoids calculating shas unless they're requested by the neo4j.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            neo4j_mapping['sha256:{}'.format(
                shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom NEO4J_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('NEO4J_'):
                # 'NEO4J_SOME_KEY' => 'some-key'
                neo4j_mapping[env_key[len('NEO4J_'):].lower().replace(
                    '_', '-')] = env_val

        return neo4j_mapping

    def _apply_templating_to_file(self, filename, orig_content):
        neo4j_mapping = self._get_neo4j_mapping_for_content(orig_content)
        new_content = orig_content
        for neo4j_key, neo4j_val in neo4j_mapping.items():
            new_content = new_content.replace('{{%s}}' % neo4j_key,
                                              neo4j_val)
        ...
        neo4j_keys = list(neo4j_mapping.keys())
        neo4j_keys.sort()
        for key in neo4j_keys:
            logger.info('  {{%s}} => %s' % (key, neo4j_mapping[key]))
