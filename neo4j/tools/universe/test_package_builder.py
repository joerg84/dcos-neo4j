...
def test_neo4j_service_(mocker):
    ...
    package = Package("neo4j", "stub-universe")
    package_manager = mocker.Mock()

    package_manager.get_latest = mocker.MagicMock(return_value=Package.from_json(package_json))

    upb = UniversePackageBuilder(package, package_manager, 'resources/neo4j', ',', [])

    neo4j_mapping = upb._get_neo4j_mapping_for_content("")
    assert 'upgrades-from' in neo4j_mapping
    assert neo4j_mapping['upgrades-from'] == "1.2.3"
